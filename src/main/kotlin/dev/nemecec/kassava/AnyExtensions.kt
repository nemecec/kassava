 ...
 * @param superEquals lambda for calling super.equals() if required (if used, all classes involved
 *   should implement the [SupportsMixedTypeEquality] interface)
 * @param T the type of the receiving class
 */
public inline fun <reified T : Any> T.kotlinEquals(
  other: Any?,
 ...
 * @param T the type of the receiving class
 */
public inline fun <reified T : Any> T.kotlinHashCode(
  properties: Array<out KProperty1<T, Any?>>,
  noinline superHashCode: (() -> Int)? = null,
): Int {
  val values =
    Array(properties.size) { i ->
 ...
 *
 * The implementation is based on the implementation provided by Guava's ToStringHelper
 ...
 * @param T the type of the receiving class
 */
public inline fun <reified T : Any> T.kotlinToString(
  properties: Array<out KProperty1<T, Any?>>,
