...
class ToStringTest {
  ...
  fun `a fully populated person has the correct string representation`() {
    val person =
      Person(
        name = "Jim",
        age = 31,
        address = Person.Address(streetNumber = 123, streetName = "Sesame", country = "US"),
      )
  ...
  fun `a person with only mandatory properties has the correct string representation`() {
    val person = Person(name = "Jim", address = Person.Address(country = "US"))
  ...
  fun `a person with omitNulls enabled and only mandatory properties has the correct string representation`() {
    val person = PersonOmitNulls(name = "Jim", address = PersonOmitNulls.Address(country = "US"))
  ...
  fun `an object with a 2D array has the correct string representation`() {
    val value =
      ClassWithArray(array = arrayOf(arrayOf(1, 2, 3), null, arrayOf(4, 5, 6), arrayOf(7, 8, 9)))
...
private open class Person(val name: String, val age: Int? = null, val address: Address) {
  ...
  override fun toString() =
    kotlinToString(properties = arrayOf(Person::name, Person::age, Person::address))
